...
4. With imputed missing values, the mean number of steps taken per day is `r round(meanSteps,2)` and the median is `r round(medianSteps,2)`. Is `r medianSteps` right? There are `r nrow(foo)` observations. Of these, `r zeroSteps` record 0 steps. That's `r round(100*zeroSteps/nrow(foo),2)`%, so it looks right. It appears that imputing missing values has a negligible effect on the mean and no effect at all on the median, because zero values amount to more than half of all observations. The new histogram is below:

    ```{r newhistogram, echo=FALSE}
    hist(foo$steps)
    ```
...
2. Plot it:
    ...
    ```{r plotweekends, echo=FALSE, message=FALSE}
    # Panel plot
    require(ggplot2)
